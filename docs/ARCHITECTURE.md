...
{ shape, map, mat, x, y, angle, q,            // 월드 위치·자세 + 던지기 품질(미끄덩·분산에 사용)
  pads: [{id, def, stuck, hp, hp0, rj}],      // 패드별 그립 HP(기하 계수×품질 지터) + 감쇠지터
...
- **난수·다이나믹스**: 부착 시 ①기하 계수(결정론 — 가로 접선·스핀이 패드별 HP 0.72~1.28 비대칭, `resolveImpact`의 geoF) ②품질 연동 지터(퍼펙트 ±15% ~ 슬로피 ±33%, 주스 ±5~15%) ③hold 중 저품질 미끄덩 이벤트(SLIP_RATE×(1−q)×경과). 롤 방향: 매 롤 확률 샘플링 — 커브 바이어스(롤마다 감쇠)+좌우 HP 비대칭+직전 방향 관성, P 클램프 [0.15,0.85] (영구 고정 없음, 지그재그).
