...
- **난수·다이나믹스**: 부착 시 ①기하 계수(결정론 — 가로 접선·스핀이 패드별 HP 0.72~1.28 비대칭, `resolveImpact`의 geoF) ②품질 연동 지터(퍼펙트 ±15% ~ 슬로피 ±33%, 주스 ±5~15%) ③hold 중 저품질 미끄덩 이벤트(SLIP_RATE×(1−q)×경과). 롤 방향: 커브 고정(|driftX|>0.006) > 남은 패드 좌우 HP 비대칭(약한 쪽) > 랜덤.
