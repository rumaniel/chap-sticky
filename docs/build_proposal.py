...
y = body(y, [
    ...
    '- 잡은 채 빙글빙글 돌리면 스핀 장전 — 놓는 순간 팽그르르 휘어져 날아가는 커브볼',
    '- 너무 세면 튕겨 나가고, 너무 약하면 바닥에 철퍼덕 — 스테이지마다 잘 붙는 세기가 다르다',
    '- 부분 궤적 미리보기 + 파워 게이지(초록=부착 존) + 가이드 손 — 설명 없는 5초 온보딩',
    '',
    '**② 부착 — 끈적 패드 단위 판정',
    '- 충돌 순간 접촉한 끈적 패드(손·발·다리끝)마다 닿은 곳의 벽 재질로 개별 판정',
    '- 창틀·테두리 같은 미끈한 곳에 닿은 패드는 잡지 못한다 (한 벽에도 여러 재질)',
    '',
    '**③ 크롤다운 — 그립이 모든 것을 결정',
    '- 패드마다 그립 HP: 위쪽 패드(장력 집중)부터 뽁뽁 벗겨진다',
    '- 지지가 무너지면 남은 손발을 축으로 기우뚱 — 천천히 기울다 휙 하고 넘어간다',
...
crit = [
    ...
    ('발전 가능성', [
        '데이터 주도 설계: 모형(끈적 패드)·맵(재질 존) 레지스트리에 정의만 추가하면 확장',
...
# 기술 스펙 박스 — 상하좌우 여백 넉넉히
y -= 4 * mm
c.setFillColor(PANEL)
c.roundRect(M, y - 40 * mm, W - 2 * M, 38 * mm, 3 * mm, stroke=0, fill=1)
c.setFillColor(YELL)
c.setFont('MalgunB', 10.5)
c.drawString(M + 7 * mm, y - 9.5 * mm, '기술 스펙')
sy = y - 18 * mm
for lab, ln in [
    ('렌더링', 'HTML5 Canvas 2D, 가상 480×800 레터박스, DPR 대응  |  사운드: WebAudio 신스(저작권 클린)'),
    ('물리', '자체 구현 — 포사체·커브·패드 그립 HP·롤·진자 필오프  |  저장: localStorage'),
    ('코드', '순수 JS 11모듈 약 3,400줄, 빌드 과정 없음  |  저장소: github.com/rumaniel/chap-sticky (공개)'),
]:
    c.setFillColor(SOFT)
    c.setFont('MalgunB', 9)
    c.drawString(M + 7 * mm, sy, lab)
    c.setFillColor(INK)
    c.setFont('Malgun', 9)
    c.drawString(M + 24 * mm, sy, ln)
    sy -= 7 * mm
