...
- 2026-09-01: R10·R11 완료(fix/balance-perfect-gauge). 퍼펙트 판정을 `bell()` 품질 대신 충격비 밴드(`PERFECT_BAND=0.035`)로 바꿔 78% -> 18%. 게이지 접선 보정을 비례식(`TANGENT_DIV=1.20`)으로 바꿔 표시 limit 이 실제 부착 한계를 넘지 않게 했다 — 부작용으로 초록 존을 따르는 플레이어의 부착률이 칠판 97.6% / 거실 98.8% / 냉장고 96.4% 로 올랐다.
>>>>>>> 232a2c1 (fix: 퍼펙트 판정 폭 축소 + 게이지 한계 표시 정정 (R10, R11))
