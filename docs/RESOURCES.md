...
| R8 | 앱 아이콘·스플래시 | Android 런처 아이콘(적응형 포함)과 스플래시를 `shapes.js` 드로잉 코드로 렌더해 생성. 생성기는 `tools/gen_android_assets.py` | ~70분 | 저 | **완료 2026-08-25** |
...
게임은 에셋 파일이 0개라 소스 아트가 없다. 그래서 **게임 자신의 드로잉 코드**
(`shapes.js` 의 `man.draw()`)로 렌더해서 만들었다 — 인게임 찐득이와 아이콘이
같은 코드에서 나오므로 어긋날 수가 없다.

| 요소 | 내용 | 평가 |
|---|---|---|
| 소스 아트 | `assets/icon.png`(1024) · `assets/icon-foreground.png`(1024, 적응형 안전영역 66% 준수) · `assets/splash.png`(2732) | ✅ R8 |
| 아이콘 | 벽에 눌린(squash 0.84) 찐득이 + 접착 자국. 적응형 배경은 단색 `#2A2145` | ✅ R8 |
| 스플래시 | 평면 `#1a1530` 배경 + 찐득이 + 워드마크(로고와 같은 초록 그라디언트) | ✅ R8 |
| 생성기 | `tools/gen_android_assets.py` — Pillow 로 리사이즈·원형마스크·CENTER_CROP. `@capacitor/assets`(sharp 네이티브 30MB) 대신 | ✅ R8 |
| 밀도 세트 | 런처 5종 × (일반·원형·적응형 전경), 세로 스플래시 5종 + 기본 1종 = **22 파일 470KB** | ✅ |

세로 고정 앱이라 `drawable-land-*` 는 만들지 않는다 — 표시될 일이 없는데 APK 만
2MB 넘게 불린다. 스플래시 배경도 그라디언트에서 평면으로 바꿔 4.4MB → 470KB.
