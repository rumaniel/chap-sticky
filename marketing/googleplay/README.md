...
| # | 자산 | 스펙 | 상태 |
|---|---|---|---|
| S1 | 앱 아이콘 | 512×512 PNG (32bit, 알파 가능하나 투명 배경 금지), ≤1MB | ⬜ 미생성 — `assets/icon.png`(1024) 축소 |
| S2 | 그래픽 이미지(피처) | 1024×500 PNG(24bit, 알파 없음) 또는 JPEG, ≤15MB | ⬜ 미생성 |
| S3 | 휴대전화 스크린샷 | **최소 4장** (최대 8장), 1080×1920 권장, 각 변 320~3840px, ≤8MB | ⬜ 재렌더 필요 (아래 주의) |
...
**S3 주의 — 기존 스크린샷을 그대로 못 쓴다.** `docs/shots/`와 `marketing/itchio/`의
스크린샷은 **960×1600 (3:5, 0.600)** 인데 Play 는 **9:16 (0.5625)** 을 요구한다.
1080×1920 으로 다시 렌더해야 한다. 게임 가상 해상도가 480×800(3:5)이라 9:16 프레임에서는
위아래에 각 60px 씩 레터박스가 생기는데, 배경색 `#1a1530` 으로 채워지므로 자연스럽다.
...
가로 배너다. 스토어 상단·추천 배치에 쓰인다. 구성안:

- 배경: 게임 벽 톤 `#2a2145` → `#1a1530` 그라디언트
- 좌측: 워드마크 "찹! 찐득이" (로고 초록 그라디언트) + "SPLAT! STICKY"
- 우측: 벽에 눌린 찐득이 + 접착 자국 (아이콘과 같은 `shapes.js` 드로잉)
- 텍스트는 중앙 866×250 안쪽에 — 기기에 따라 가장자리가 잘린다
...
**콘솔 (사람이 직접)**
- [ ] Play Console 에서 앱 생성 (§1)
- [ ] `studio-mangru.github.io/privacy/` 에 이 앱 추가
- [ ] 앱 콘텐츠 선언 전부 작성 (§4)
- [ ] 스토어 등록정보 텍스트·이미지 업로드 (§2, §3)
- [ ] 스토어 설정 (§5)
- [ ] 내부 테스트 트랙에 테스터 추가

**저장소 (자동화 가능)**
- [ ] S1 앱 아이콘 512 생성
- [ ] S2 피처 그래픽 1024×500 생성
- [ ] S3 스크린샷 1080×1920 5장 재렌더
