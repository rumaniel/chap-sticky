...
실제 물리 코드를 헤드리스로 돌렸다. 플레이어를 3단계로 모델링해
릴리즈 높이 · 플릭 속도 · 조준 각도 · 스핀 사용률에 정규분포 오차를 준다.

| 스킬 | 릴리즈 y | 속도 배수 | 각도 σ | 스핀 사용 |
|---|---|---|---|---|
| 초보 | 610 ± 55 | 1.25 ± 0.30 | 13° | 10% |
| 평균 | 560 ± 50 | 1.05 ± 0.18 | 8° | 28% |
| 숙련 | 520 ± 35 | 1.00 ± 0.09 | 4° | 55% |
...
|---|---|---|---|---|---|---|---|
| 칠판 | 97.7% | 74.3% | 27.0% | **0%** | 10.7s | 13.2s | 663 |
| 거실 | 94.7% | 81.0% | 25.7% | **0%** | 8.3s | 9.8s | 553 |
| 냉장고 | 88.3% | 78.0% | 24.0% | 4.0% | 8.1s | 10.1s | 425 |
| 유리창 | 58.0% | 29.3% | 15.3% | 0.3% | 2.9s | 6.1s | 93 |

모형별 버티기 (칠판, 평균 플레이어):

| 모형 | p50 | p90 | 최대 | 감쇠 |
|---|---|---|---|---|
| 찐득맨 | 10.6s | 11.9s | 14.0s | 1.0 |
| **문어찐득** | **13.4s** | **14.6s** | **16.7s** | 0.85 |
| 별찐득 | 11.7s | 12.9s | 14.0s | 1.2 |
...
| `ACH_PERFECT_15` | 찰떡같이 | 퍼펙트 **15회** | 증분 15 | 퍼펙트 34~91%/던지기 (§4 측정) → 약 4라운드 |
...
| 항목 | 원안 | 확정 | 이유 |
|---|---|---|---|
| 버티기 | 30초 / 60초 | **10초 / 15초** | 물리 최대가 16.7초 |
| 퍼펙트 | 10회 | **15회** | 퍼펙트율을 78%→18% 로 조정(R10). 그 기준으로 약 17라운드 |
...
1. ~~**칠판 보너스 스팟 2개가 도달 불가**~~ → **2026-09-01 수정 완료(R9)**.
   스팟을 도달 가능 영역 안으로 옮겨 접촉률이 칠판 0%→25.7%, 거실 0%→25.3%,
   유리창 0.3%→24.3%, 냉장고 4.0%→49.7% 가 됐다
2. ~~**"퍼펙트"가 78%**~~ → **2026-09-01 수정 완료(R10)**. 충격비 밴드 판정으로 바꿔 **18%**
   (칠판 10% / 거실 19.6% / 유리창 6.4% / 냉장고 37.2%). 냉장고만 여전히 높은데 그 맵은
   충격비가 SWEET 부근에 몰린다 — 별도 과제로 남긴다
3. ~~**게이지 limit 표시가 실제보다 높다**~~ → **2026-09-01 수정 완료(R11)**. 접선 보정을
   비례식으로 바꿔 4개 맵 모두 표시 ≤ 실제. 초록 존을 따르면 칠판·거실·냉장고에서 96~99% 붙는다
4. **해금이 약 8분에 끝난다** — 칠판 라운드당 ~3,300점, 15,000점까지 5라운드
