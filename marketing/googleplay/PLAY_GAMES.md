...
`node tools/sim/sim.js 300` 으로 그대로 재현되고, `tools/sim/baseline.json` 에 스냅샷으로
저장돼 있다. 물리를 바꾸면 `node tools/sim/sim.js 300 --check` 가 먼저 깨진다 — 의도한
변경이면 `--update` 로 스냅샷을 갱신하고 이 문서를 고친다. 문서가 조용히 낡는 걸 막는 장치다.
...
> 세기를 **절대값 1.05 px/ms 로 고정**했다. 당시 게이지(SWEET_DIV=1.06) 기준 맵별 sweet 은
> 칠판 1.69 / 거실 1.30 / 유리창 1.11 / 냉장고 1.17 이라 냉장고만 우연히 근처였고, 그래서
> "냉장고만 퍼펙트율 37%" 라는 잘못된 결론이 나왔다 — 게임 밸런스가 아니라 모델 아티팩트였다.
> (현재 sweet 은 닫힌 해라 값이 다르다: 칠판 1.63 / 거실 1.35 / 유리창·냉장고 1.18)

### 평균 플레이어 실측 (300회/맵, 시드 12345, 찐득맨)

| 맵 | 부착률 | 퍼펙트율 | 커브부착률 | 스팟률 | 버티기 p50 | 최대 | 던지기당 점수 |
|---|---|---|---|---|---|---|---|
| 칠판 | 100.0% | 25.0% | 17.7% | 27.0% | 10.6s | 12.7s | 600 |
| 거실 | 97.7% | 22.7% | 17.3% | 28.0% | 8.3s | 9.8s | 482 |
| 냉장고 | 92.0% | 18.0% | 23.7% | 49.0% | 8.2s | 10.0s | 461 |
| 유리창 | 70.3% | 8.7% | 16.3% | 32.0% | 4.2s | 6.3s | 125 |

모형별 버티기 (칠판, 평균 플레이어):

| 모형 | p50 | p90 | 최대 | 감쇠 |
|---|---|---|---|---|
| 찐득맨 | 10.6s | 11.7s | 12.7s | 1.0 |
| **문어찐득** | **13.5s** | **14.6s** | **16.1s** | 0.85 |
| 별찐득 | 12.0s | 13.2s | 14.3s | 1.2 |

**전 구간 최대 버티기 = 16.1초** (문어 × 칠판). 원안의 30초·60초는 **물리적으로 불가능**했다.
...
| `ACH_GLASS` | 유리창 개방 | 누적 4,000점 | 일반 | 평균 2라운드 (R12) |
| `ACH_OCTO` | 문어 등장 | 누적 10,000점 | 일반 | 평균 4라운드 |
| `ACH_FRIDGE` | 냉장고 습격 | 누적 25,000점 | 일반 | 평균 8라운드 |
| `ACH_STAR` | 별의 순간 | 누적 45,000점 | 일반 | 평균 13라운드 · 초보 17 · 숙련 12 |
| `ACH_HOLD_10` | 10초의 벽 | 한 번에 **10초** 버티기 | 일반 | 칠판 p50 = 10.6s — 절반이 달성 |
| `ACH_HOLD_15` | 요지부동 | 한 번에 **15초** 버티기 | 히든 | **문어찐득이 사실상 필수** (맨 최대 12.7s, 문어 최대 16.1s·p90 14.6s) |
| `ACH_CURVE_15` | 커브 마스터 | 커브볼로 **15번** 붙이기 | 증분 15 | 커브부착 16~24%/던지기 → 약 13~18라운드 |
| `ACH_PERFECT_15` | 찰떡같이 | 퍼펙트 **15회** | 증분 15 | 퍼펙트율 18~25%/던지기(유리창만 9%) → 약 **12~17라운드** |
| `ACH_SPOT_R3` | 스팟 헌터 | **한 라운드(5구)에 스팟 3개** | 일반 | 라운드의 13%(칠판)~48%(냉장고) |
| `ACH_PARTY_8` | 파티의 밤 | 8인 파티 완주 | 히든 | 사람 8명 필요 |
| `ACH_ALL_MAPS` | 벽 정복 | 4개 맵 전부 완주 | 증분 4 | 냉장고(25,000) 해금 이후 자연 달성 |
...
| 퍼펙트 | 10회 | **15회** | 판정을 충격비 밴드로 교체(R10) + 밴드를 조합별 민감도 비례로(R15). 18~25%/던지기라 12~17라운드 |
| 커브 | 25회 | **15회** | 25회는 19라운드로 너무 길다 |
| 스팟 | 한 라운드 5개 | **한 라운드 3개** | R9·R13 이후 접촉률이 맵별 27~49%/던지기. P(5구 중 3개↑)는 칠판 12.6% / 거실 13.8% / 유리창 19.1% / 냉장고 48.1%. 맵은 플레이어가 고르므로 **실질 난이도는 냉장고 기준 약 48%** 다 |
...
1. ~~**칠판 보너스 스팟 2개가 도달 불가**~~ → **2026-09-01 수정 완료(R9)**.
   스팟을 도달 가능 영역 안으로 옮겨 접촉률이 칠판 27.0% / 거실 28.0% /
   유리창 32.0% / 냉장고 49.0% 가 됐다
2. ~~**"퍼펙트"가 78%**~~ → **2026-09-01 수정 완료(R10)**. `q > 0.9` 는 충격비로 `r`
   0.42~0.79 라 부착 범위의 37% 였다. `\|r − SWEET\| < 0.035` 고정 밴드로 교체했고,
   R15 에서 조합별 가변 밴드로 다시 바꿨다 (아래 6번)
3. ~~**게이지 limit 표시가 실제보다 높다**~~ → **2026-09-01 수정(R11) + 09-02 보강(R14·R15)**.
   접선 보정을 비례식으로 바꾸고(R11), limit 과 sweet 의 상수를 분리했다가(R14), sweet 은
   `r(s)=SWEET` 의 닫힌 해로 바꿨다(R15). 검증: 28,941회 스윕에서 표시 limit 이하 튕김 0회
4. ~~**해금이 약 8분에 끝난다**~~ → **2026-09-02 수정 완료(R12)**. 임계값
   3k/5k/12k/15k → 4k/10k/25k/45k. 첫 해금은 2라운드 유지, 전부 해금은 평균 13라운드(20분).
   파티 모드 누적도 전원 합 → 최고 1인으로 바꿨다
...
6. ~~**퍼펙트 밴드 폭이 맵마다 다르다**~~ → **2026-09-02 수정 완료(R15)**. 밴드를
   조합(모형×맵)당 sweet 지점 민감도에 비례시키고 `sweet` 을 닫힌 해로 교체. 벽 충돌은
   고정 타임스텝 누적기로 주사율·히치 무관. 퍼펙트율 22/24/13/62% → **25/23/9/18%**, 세기 기준
   퍼펙트 폭 7.9~8.9%(별×유리창만 4.9%), 마커대로 던졌을 때 퍼펙트가 나오는 조합 88% → **100%**
