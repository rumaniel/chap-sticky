...
| `ACH_PERFECT_50` | 찰떡같이 | 퍼펙트 **50회** | 증분 50 | 퍼펙트 78%/던지기 → 약 13라운드 |
...
| 퍼펙트 | 10회 | **50회** | 퍼펙트율이 78% — 10회는 2라운드면 끝 |
...
증분형 3개(`CURVE_15` / `PERFECT_50` / `ALL_MAPS`)도 **카운터는 로컬에 둔다.**
...
2. **"퍼펙트"가 78%** — 평균 플레이어가 4번 중 3번 받는다. 이름값을 못 한다
3. **게이지 limit 표시가 실제보다 높다** — 냉장고 표시 1.91 인데 1.8 부터 전부 튕긴다 (약 11% 차이).
   코드 주석에 "안내용 근사치" 라고 적혀 있긴 하다
