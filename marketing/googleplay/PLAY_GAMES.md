...
| 냉장고 | 88.7% | 17.7% | 23.3% | 47.0% | 8.3s | 9.9s | 456 |
...
| **문어찐득** | **13.5s** | **14.7s** | **15.9s** | 0.85 |
| 별찐득 | 12.0s | 13.1s | 14.3s | 1.2 |

**전 구간 최대 버티기 = 15.9초** (문어 × 칠판). 원안의 30초·60초는 **물리적으로 불가능**했다.
...
| `ACH_HOLD_15` | 요지부동 | 한 번에 **15초** 버티기 | 히든 | **문어찐득이 사실상 필수** (맨 최대 12.7s, 문어 최대 15.9s·p90 14.7s) |
| `ACH_CURVE_15` | 커브 마스터 | 커브볼로 **15번** 붙이기 | 증분 15 | 커브부착 16~23%/던지기 → 약 13~18라운드 |
| `ACH_PERFECT_15` | 찰떡같이 | 퍼펙트 **15회** | 증분 15 | 퍼펙트율 18~25%/던지기(유리창만 9%) → 약 **12~17라운드** |
| `ACH_SPOT_R3` | 스팟 헌터 | **한 라운드(5구)에 스팟 3개** | 일반 | 라운드의 13%(칠판)~44%(냉장고) |
...
| 항목 | 원안 | 확정 | 이유 |
|---|---|---|---|
| 버티기 | 30초 / 60초 | **10초 / 15초** | 물리 최대가 15.9초 (문어 × 칠판) |
| 퍼펙트 | 10회 | **15회** | 판정을 충격비 밴드로 교체(R10) + 밴드를 조합별 민감도 비례로(R15). 18~25%/던지기라 12~17라운드 |
| 커브 | 25회 | **15회** | 25회는 19라운드로 너무 길다 |
| 스팟 | 한 라운드 5개 | **한 라운드 3개** | R9·R13 이후 접촉률이 맵별 27~47%/던지기. P(5구 중 3개↑)는 칠판 12.6% / 거실 13.8% / 유리창 19.1% / 냉장고 44.4%. 맵은 플레이어가 고르므로 **실질 난이도는 냉장고 기준 약 44%** 다 |
