...
|---|---|---|---|---|---|---|---|
| 칠판 | 100.0% | 22.3% | 19.3% | 27.3% | 10.9s | 12.7s | 611 |
| 거실 | 94.7% | 23.7% | 19.0% | 32.7% | 8.4s | 9.7s | 460 |
| 냉장고 | 90.0% | **62.0%** | 15.7% | 44.3% | 8.3s | 10.2s | 522 |
| 유리창 | 64.7% | 13.0% | 11.7% | 28.7% | 4.1s | 6.1s | 120 |

모형별 버티기 (칠판, 평균 플레이어):

| 모형 | p50 | p90 | 최대 | 감쇠 |
|---|---|---|---|---|
| 찐득맨 | 10.9s | 11.9s | 12.7s | 1.0 |
| **문어찐득** | **13.5s** | **14.4s** | **16.1s** | 0.85 |
| 별찐득 | 12.0s | 13.1s | 14.4s | 1.2 |

**전 구간 최대 버티기 = 16.1초** (문어 × 칠판). 원안의 30초·60초는 **물리적으로 불가능**했다.

냉장고 퍼펙트율만 62% 로 튄다. 원인은 확인됐다 — `PERFECT_BAND` 는 충격비 `r` 공간에서
정의되는데 `r` 이 세기에 반응하는 기울기가 맵마다 달라, 같은 밴드가 **세기로는** 칠판
8.1% / 거실 7.7% / 냉장고 28.0% 폭이 된다 (`node tools/sim/gauge.js` 검사 3).
밴드를 맵별로 정규화할지는 별건이다 → R15.
...
| `ACH_PERFECT_15` | 찰떡같이 | 퍼펙트 **15회** | 증분 15 | 퍼펙트율 13~62%/던지기(맵별) → 칠판 약 13라운드, 냉장고 약 5라운드 |
| `ACH_SPOT_R3` | 스팟 헌터 | **한 라운드(5구)에 스팟 3개** | 일반 | 맵에 따라 라운드의 13%(칠판)~39%(냉장고) |
...
| 퍼펙트 | 10회 | **15회** | 판정을 충격비 밴드로 교체(R10). 맵별 13~62%/던지기라 5~13라운드 |
...
1. ~~**칠판 보너스 스팟 2개가 도달 불가**~~ → **2026-09-01 수정 완료(R9)**.
   스팟을 도달 가능 영역 안으로 옮겨 접촉률이 칠판 27.3% / 거실 32.7% /
   유리창 28.7% / 냉장고 44.3% 가 됐다
...
6. **퍼펙트 밴드 폭이 맵마다 다르다** — `r` 공간 정의라 세기로는 칠판 8.1% /
   냉장고 28.0%. 냉장고 퍼펙트율만 62% 인 이유다 (R15, 미착수)
