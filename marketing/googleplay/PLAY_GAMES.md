...
| `ACH_SPOT_3` | 스팟 헌터 | 보너스 스팟 **통산 3개** | 증분 3 | 냉장고에서만 현실적 (4%) |
...
| 스팟 | 한 라운드 5개 | **통산 3개** | 한 던지기 최대 1개, 칠판·거실은 0% |

증분형 4개(`CURVE_15` / `PERFECT_50` / `SPOT_3` / `ALL_MAPS`)도 **카운터는 로컬에 둔다.**
...
1. **칠판 보너스 스팟 2개가 도달 불가** — 릴리즈 높이·속도·각도를 1,512조합 훑어서 **0회**.
   거실도 2.1%, 유리창 1.5%. 냉장고만 23.5% 로 정상 동작한다
